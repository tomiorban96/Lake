...
//namespace backtrack{
   ...
   void pa4(void){/*Visszaállítás */
    ...
    int ft1(void){/* A célt elértük? */
        if (actualRoute.passedTime > 0 && actualRoute.position->name == "Graphiváros" && actualRoute.passedTime <= T) return 1; //return 0 lesz
        return 0;
    }
...
int main(int argc, char **argv){
   ...
   cout << actualRoute.position->lines.size()+1;
   //cout << actualRoute.position->lines[1].next->name<<endl;
